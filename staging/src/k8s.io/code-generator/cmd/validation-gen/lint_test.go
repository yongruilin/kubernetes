...
func TestRuleOptionalAndRequired(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			comments: []string{"+optional", "+required"},
			wantMsg:  "comments slice contains conflicting tags: {+optional, +required}",
			wantErr:  false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ruleOptionalAndRequired() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if msg != tt.wantMsg {
...
func TestRuleRequiredAndDefault(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			comments: []string{"+required", "+default=somevalue"},
			wantMsg:  "comments slice contains conflicting tags: {+required, +default}",
			wantErr:  false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ruleRequiredAndDefault() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if msg != tt.wantMsg {
...
func TestConflictingTagsRule(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tags:     []string{"+tag1", "+tag2"},
			wantMsg:  "comments slice contains conflicting tags: {+tag1, +tag2}",
		},
		{
			name:     "3 tags",
			comments: []string{"tag1", "+tag2", "+tag3=value"},
			tags:     []string{"+tag1", "+tag2", "+tag3"},
			wantMsg:  "comments slice contains conflicting tags: {+tag1, +tag2, +tag3}",
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			typeToLint: &types.Type{
				...
				Kind:         types.Pointer,
				Elem:         &types.Type{Name: types.Name{Package: "testpkg", Name: "ElemType"}, CommentLines: []string{"+optional"}},
				CommentLines: []string{"+optional"},
			},
			wantCount:   2,
			expectError: false,
		...
		},
		{
			...
			typeToLint: &types.Type{
				...
				Kind: types.Map,
				Key:  &types.Type{Name: types.Name{Package: "testpkg", Name: "KeyType"}, CommentLines: []string{"+required"}},
				Elem: &types.Type{
					Name:         types.Name{Package: "testpkg", Name: "PointerElem"},
					Kind:         types.Pointer,
					Elem:         &types.Type{Name: types.Name{Package: "testpkg", Name: "ElemType"}, CommentLines: []string{"+optional"}},
