...
var (
	...
	fieldErrorListType     = types.Name{Package: fieldPkg, Name: "ErrorList"}
	fieldInternalErrorFunc = types.Name{Package: fieldPkg, Name: "InternalError"}
	...
	safeNewListMapFunc     = types.Name{Package: safePkg, Name: "NewListMap"}
	safeFieldFunc          = types.Name{Package: safePkg, Name: "Field"}
	safeLookupFunc         = types.Name{Package: safePkg, Name: "Lookup"}
	safeLookupOrZeroFunc   = types.Name{Package: safePkg, Name: "LookupOrZero"}
	safeCastFunc           = types.Name{Package: safePkg, Name: "Cast"}
	operationPkg           = "k8s.io/apimachinery/pkg/api/operation"
	operationContextType   = types.Name{Package: operationPkg, Name: "Context"}
	operationUpdateValue   = types.Name{Package: operationPkg, Name: "Update"}
)
...
func (g *genValidations) emitRegisterFunction(c *generator.Context, schemeRegistry types.Name, sw *generator.SnippetWriter) {
	...
	for _, t := range g.rootTypes {
		...
		targs := generator.Args{
			...
			"fieldErrorList":     c.Universe.Type(fieldErrorListType),
			"fieldInternalError": c.Universe.Type(fieldInternalErrorFunc),
			"fieldPath":          c.Universe.Type(fieldPathType),
			"fmtErrorf":          c.Universe.Type(fmtErrorfFunc),
			"operationContext":   c.Universe.Type(operationContextType),
			"safeCast":           c.Universe.Type(safeCastFunc),
			"safeField":          c.Universe.Type(safeFieldFunc),
		}
		...
		sw.Do("scheme.AddValidationFunc((*$.rootType|raw$)(nil), func(opCtx $.operationContext|raw$, obj, oldObj interface{}, subresources ...string) $.fieldErrorList|raw$ {\n", targs)
		sw.Do("  if len(subresources) == 0 {\n", targs)
		sw.Do("    return $.rootType|objectvalidationfn$(opCtx, obj.(*$.rootType|raw$), $.safeCast|raw$[$.rootType|raw$](oldObj), nil)\n", targs)
		sw.Do("  }\n", targs)
		...
		if statusType != nil {
			...
			if g.hasValidations(g.discovered.typeNodes[statusType]) {
				...
				sw.Do("    return $.statusType|objectvalidationfn$(opCtx, &root.$.statusField$, $.safeField|raw$($.safeCast|raw$[$.rootType|raw$](oldObj), func(oldObj $.rootType|raw$) $.statusType|raw$ { return oldObj.$.statusField$ }), nil)\n", targs)
			} else {
				sw.Do("    return nil // $.statusType|raw$ has no validation\n", targs)
			}
			sw.Do("  }\n", targs)
		}
		sw.Do("  return $.fieldErrorList|raw${$.fieldInternalError|raw$(nil, $.fmtErrorf|raw$(\"no validation found for %T, subresources: %v\", obj, subresources))}\n", targs)
		sw.Do("})\n", targs)
...
func (g *genValidations) emitValidationFunction(c *generator.Context, t *types.Type, sw *generator.SnippetWriter) {
	...
	sw.Do("func $.inType|objectvalidationfn$(opCtx $.operationContext|raw$, obj, oldObj *$.inType|raw$, fldPath *$.fieldPath|raw$) (errs $.fieldErrorList|raw$) {\n", targs)
	fakeChild := &childNode{
		node:      node,
		childType: t,
	}
...
func (g *genValidations) emitValidationForChild(c *generator.Context, thisChild *childNode, objIsPtr bool, sw *generator.SnippetWriter) {
	...
	targs := generator.Args{
		...
		"fieldErrorList":   c.Universe.Type(fieldErrorListType),
		"fieldPath":        c.Universe.Type(fieldPathType),
		"safeNewListMap":   c.Universe.Type(safeNewListMapFunc),
		"safeLookup":       c.Universe.Type(safeLookupFunc),
		"safeLookupOrZero": c.Universe.Type(safeLookupOrZeroFunc),
		"safeField":        c.Universe.Type(safeFieldFunc),
	}
	...
	case types.Struct:
		for _, fld := range thisNode.fields {
			...
			if buf.Len() > 0 {
				...
				sw.Do("  func(obj $.fieldType|raw$, oldObj $.oldDeref$$.fieldType|raw$, fldPath *$.fieldPath|raw$) (errs $.fieldErrorList|raw$) {\n", targs)
				sw.Append(buf)
				sw.Do("    return\n", targs)
				sw.Do("  }(obj.$.fieldName$, $.safeField|raw$(oldObj, func(oldObj $.inType|raw$) $.fieldDeref$$.fieldType|raw$ { return $.fieldAddr$oldObj.$.fieldName$ }), fldPath.Child(\"$.fieldJSON$\"))...)\n", targs)
				sw.Do("\n", nil)
			} else {
	...
	case types.Slice, types.Array:
		...
		if elemBuf.Len() > 0 {
			...
			// +listMapKey values.
			if isCorrelatable {
				sw.Do("oldListMap := $.safeNewListMap|raw$(oldObj, func(o $.elemType|raw$) any {", targs)
				sw.Do(" return ", targs)
			...
			sw.Do("  errs = append(errs,\n", targs)
			sw.Do("    func(obj $.elemType|raw$, oldObj $.elemDeref$$.elemType|raw$, fldPath *$.fieldPath|raw$) (errs $.fieldErrorList|raw$) {\n", targs)
			sw.Append(elemBuf)
			sw.Do("      return\n", targs)
	...
	case types.Map:
		...
		if keyBuf.Len()+valBuf.Len() > 0 {
			...
			if keyBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.keyType|raw$, oldObj $.oldKeyDeref$$.keyType|raw$, fldPath *$.fieldPath|raw$) (errs $.fieldErrorList|raw$) {\n", targs)
				sw.Append(keyBuf)
				sw.Do("      return\n", targs)
			...
			if valBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.valType|raw$, oldObj $.oldValDeref$$.valType|raw$, fldPath *$.fieldPath|raw$) (errs $.fieldErrorList|raw$) {\n", targs)
				sw.Append(valBuf)
				sw.Do("      return\n", targs)
				sw.Do("    }(val, ", targs)
				if isNilable(inType.Elem) {
					sw.Do("$.safeLookupOrZero|raw$", targs)
				} else {
					sw.Do("$.safeLookup|raw$", targs)
				}
...
func emitCallsToValidators(c *generator.Context, validations []validators.FunctionGen, objIsPtr bool, sw *generator.SnippetWriter) {
	...
	for _, v := range validations {
		...
		targs := generator.Args{
			...
			"operationUpdate": c.Universe.Type(operationUpdateValue),
		}
		if objIsPtr && !ptrOK {
			if !insideNilCheck {
				sw.Do("if obj != nil {\n", targs)
		...
		if updateOnly {
			sw.Do("if opCtx.Operation == $.operationUpdate|raw$ ", targs)
			if !ptrOK {
