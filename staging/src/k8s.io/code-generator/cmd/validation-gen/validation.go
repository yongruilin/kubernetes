...
var (
	...
	lookupType       = types.Name{Package: safePkg, Name: "Lookup"}
	lookupOrZeroType = types.Name{Package: safePkg, Name: "LookupOrZero"}
	castType         = types.Name{Package: safePkg, Name: "Cast"}
	operationPkg     = "k8s.io/apimachinery/pkg/api/operation"
	contextType      = types.Name{Package: operationPkg, Name: "Context"}
	updateType       = types.Name{Package: operationPkg, Name: "Update"}
)
...
func (g *genValidations) emitRegisterFunction(c *generator.Context, schemeRegistry types.Name, sw *generator.SnippetWriter) {
	...
	for _, t := range g.rootTypes {
		...
		targs := generator.Args{
			...
			"context":     c.Universe.Type(contextType),
			"cast":        c.Universe.Type(castType),
			"field":       c.Universe.Type(fieldType),
		}
		...
		sw.Do("scheme.AddValidationFunc((*$.rootType|raw$)(nil), func(opCtx $.context|raw$, obj, oldObj interface{}, subresources ...string) $.errorList|raw$ {\n", targs)
		sw.Do("  if len(subresources) == 0 {\n", targs)
		sw.Do("    return $.rootType|objectvalidationfn$(opCtx, obj.(*$.rootType|raw$), $.cast|raw$[$.rootType|raw$](oldObj), nil)\n", targs)
		sw.Do("  }\n", targs)
		...
		if statusType != nil {
			...
			if g.hasValidations(g.discovered.typeNodes[statusType]) {
				...
				sw.Do("    return $.statusType|objectvalidationfn$(opCtx, &root.$.statusField$, $.field|raw$($.cast|raw$[$.rootType|raw$](oldObj), func(oldObj $.rootType|raw$) $.statusType|raw$ { return oldObj.$.statusField$ }), nil)\n", targs)
			} else {
...
func (g *genValidations) emitValidationFunction(c *generator.Context, t *types.Type, sw *generator.SnippetWriter) {
	...
	sw.Do("func $.inType|objectvalidationfn$(opCtx $.context|raw$, obj, oldObj *$.inType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
	fakeChild := &childNode{
		node:      node,
		childType: t,
	}
...
func (g *genValidations) emitValidationForChild(c *generator.Context, thisChild *childNode, objIsPtr bool, sw *generator.SnippetWriter) {
	...
	targs := generator.Args{
		...
		"errorList":    c.Universe.Type(errorListType),
		"fieldPath":    c.Universe.Type(fieldPathType),
		"newListMap":   c.Universe.Type(newListMapType),
		"lookup":       c.Universe.Type(lookupType),
		"lookupOrZero": c.Universe.Type(lookupOrZeroType),
		"field":        c.Universe.Type(fieldType),
	}
	...
	case types.Struct:
		for _, fld := range thisNode.fields {
			...
			if buf.Len() > 0 {
				...
				sw.Do("  func(obj $.fieldType|raw$, oldObj $.oldDeref$$.fieldType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(buf)
				sw.Do("    return\n", targs)
				sw.Do("  }(obj.$.fieldName$, $.field|raw$(oldObj, func(oldObj $.inType|raw$) $.fieldDeref$$.fieldType|raw$ { return $.fieldAddr$oldObj.$.fieldName$ }), fldPath.Child(\"$.fieldJSON$\"))...)\n", targs)
				sw.Do("\n", nil)
			} else {
	...
	case types.Slice, types.Array:
		...
		if elemBuf.Len() > 0 {
			...
			// +listMapKey values.
			if isCorrelatable {
				sw.Do("oldListMap := $.newListMap|raw$(oldObj, func(o $.elemType|raw$) any {", targs)
				sw.Do(" return ", targs)
			...
			sw.Do("  errs = append(errs,\n", targs)
			sw.Do("    func(obj $.elemType|raw$, oldObj $.elemDeref$$.elemType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
			sw.Append(elemBuf)
			sw.Do("      return\n", targs)
	...
	case types.Map:
		...
		if keyBuf.Len()+valBuf.Len() > 0 {
			...
			if keyBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.keyType|raw$, oldObj $.oldKeyDeref$$.keyType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(keyBuf)
				sw.Do("      return\n", targs)
			...
			if valBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.valType|raw$, oldObj $.oldValDeref$$.valType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(valBuf)
				sw.Do("      return\n", targs)
				sw.Do("    }(val, ", targs)
				if isNilable(inType.Elem) {
					sw.Do("$.lookupOrZero|raw$", targs)
				} else {
					sw.Do("$.lookup|raw$", targs)
				}
...
func emitCallsToValidators(c *generator.Context, validations []validators.FunctionGen, objIsPtr bool, sw *generator.SnippetWriter) {
	...
	for _, v := range validations {
		...
		targs := generator.Args{
			...
			"update":      c.Universe.Type(updateType),
		}
		if objIsPtr && !ptrOK {
			if !insideNilCheck {
				sw.Do("if obj != nil {\n", targs)
