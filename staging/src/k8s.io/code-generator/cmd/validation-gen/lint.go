...
func (l *linter) lintType(t *types.Type) error {
	...
	if t.CommentLines != nil {
		klog.V(4).Infof("linting type %s", t.Name.String())
	...
	case types.Struct:
		...
		for _, member := range t.Members {
			klog.V(4).Infof("linting comments for field %s of type %s", member.String(), t.Name.String())
...
func conflictingTagsRule(comments []string, tags ...string) (string, error) {
	...
	if len(tagCount) > 1 {
		return fmt.Sprintf("comments slice contains conflicting tags: {%s}", strings.Join(tags, ", ")), nil
	}
	return "", nil
}

// ruleOptionalAndRequired checks for conflicting tags +optional and +required in a slice of comments.
func ruleOptionalAndRequired(comments []string) (string, error) {
	return conflictingTagsRule(comments, "+optional", "+required")
}

// ruleRequiredAndDefault checks for conflicting tags +required and +default in a slice of comments.
func ruleRequiredAndDefault(comments []string) (string, error) {
	return conflictingTagsRule(comments, "+required", "+default")
}
